...
pub fn start_capture_with_level(
    ...
    start_capture_with_level_and_events(device_contains, gain, tx, latest_level, None)
}

pub fn start_capture_with_level_and_events(
    device_contains: &str,
    gain: f32,
    tx: Sender<Vec<i16>>,
    latest_level: Option<Arc<AtomicU32>>,
    ...
    let stream = match sample_format {
        SampleFormat::F32 => {
            build_stream::<f32>(
                ...
                tx,
                latest_level,
                stream_error_callback(event_tx.clone()),
                |value| value,
            )?
        }
        SampleFormat::I16 => {
        ...
        SampleFormat::U16 => {
            build_stream::<u16>(
                ...
                tx,
                latest_level,
                stream_error_callback(event_tx),
...
fn stream_error_callback(
    ...
    move |error| {
        ...
        if let Some(event_tx) = &event_tx {
            let _ = event_tx.try_send(AudioEvent::StreamError(error.to_string()));
        }
    }
}
