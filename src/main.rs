...
fn run_app() -> Result<()> {
    ...
    use std::sync::Arc;
    use uvox::audio::AudioEvent;
    use uvox::hotkey::{self, HotkeyEvent};
    use uvox::input::{foreground_window_id, WindowsTextSink};
    use uvox::overlay::{OverlayHandle, OverlayPrimary};
    use uvox::parakeet_native::ParakeetNative;
    use uvox::transcript::{Typist, TypistEvent, TypistFailure};
    ...
    let _capture = uvox::audio::start_capture_with_level_and_events(
    ...
    loop {
        select! {
            ...
            recv(hotkey_rx) -> message => match message {
                Ok(HotkeyEvent::HotkeyDown) if active.is_none() && typing_session.is_none() => {
                    ...
                    overlay.start_recording(target_window);
                    ...
                    if parakeet.is_none() {
                        ...
                        match ParakeetNative::load_from_config(&config) {
                            Ok(engine) => {
                                parakeet = Some(engine);
                                overlay.notify_info("Speech model ready", Some(Duration::from_millis(900)));
                                tracing::info!("native CUDA Parakeet runtime ready");
                            }
                            Err(error) => {
                                tracing::error!(%error, "native CUDA Parakeet runtime failed to load");
                                active = None;
                                typist.cancel(session_id);
                                overlay.set_primary(OverlayPrimary::Hidden);
                                overlay.notify_error("Speech model failed - see log", Duration::from_secs(7));
                        ...
                        if let Err(error) = parakeet
                            .as_mut()
                            .unwrap()
                            .load_context(&config.parakeet_model_path())
                        {
                            tracing::error!(%error, "native CUDA Parakeet context failed to reload");
                            active = None;
                            typist.cancel(session_id);
                            overlay.set_primary(OverlayPrimary::Hidden);
                            overlay.notify_error("Speech model failed - see log", Duration::from_secs(7));
                ...
                Ok(HotkeyEvent::HotkeyUp) => {
                    ...
                    if let Some(recording) = active.take() {
                        ...
                        if samples < 1_600 {
                            ...
                            overlay.set_primary(OverlayPrimary::Hidden);
                            overlay.notify_info("Recording too short", Some(Duration::from_secs(2)));
                        ...
                        let Some(engine) = &parakeet else {
                            ...
                            typist.cancel(recording.session_id);
                            overlay.set_primary(OverlayPrimary::Hidden);
                            overlay.notify_error("Speech model unavailable - see log", Duration::from_secs(7));
                            tray.set_status(TrayStatus::Error);
                            continue;
                        };
                        match engine.transcribe_pcm16_16k(&recording.samples) {
                            Ok(text) if !text.trim().is_empty() => {
                                ...
                                if foreground == recording.target_window {
                                    overlay.set_primary(OverlayPrimary::Typing);
                                    typing_session = Some(recording.session_id);
                                    ...
                                    typist.cancel(recording.session_id);
                                    overlay.set_primary(OverlayPrimary::Hidden);
                                    overlay.notify_warning("Text not inserted: active window changed", Duration::from_secs(5));
                                    tray.set_status(if config.hotkey_enabled { TrayStatus::Ready } else { TrayStatus::Disabled });
                                }
                            }
                            Ok(_) => {
                                ...
                                typist.cancel(recording.session_id);
                                overlay.set_primary(OverlayPrimary::Hidden);
                                overlay.notify_info("No speech detected", Some(Duration::from_secs(2)));
                                tray.set_status(if config.hotkey_enabled { TrayStatus::Ready } else { TrayStatus::Disabled });
                            }
                            Err(error) => {
                                ...
                                typist.cancel(recording.session_id);
                                overlay.set_primary(OverlayPrimary::Hidden);
                                overlay.notify_error("Transcription failed - see log", Duration::from_secs(7));
                                tray.set_status(TrayStatus::Error);
                            }
                        }
                    }
            ...
            recv(audio_event_rx) -> message => match message {
                Ok(AudioEvent::StreamError(error)) => {
                    tracing::error!(%error, "microphone stream reported an error");
                    if let Some(recording) = active.take() {
                        typist.cancel(recording.session_id);
                    }
                    if let Some(session_id) = typing_session.take() {
                        typist.cancel(session_id);
                    }
                    overlay.set_primary(OverlayPrimary::Hidden);
                    overlay.notify_error("Microphone error - see log", Duration::from_secs(7));
                    tray.set_status(TrayStatus::Error);
                }
                Err(_) => {}
            },
            recv(typist_rx) -> message => match message {
                Ok(TypistEvent::Completed { session_id }) => {
                    if typing_session == Some(session_id) {
                        typing_session = None;
                        overlay.hide();
                        tray.set_status(if config.hotkey_enabled { TrayStatus::Ready } else { TrayStatus::Disabled });
                        tracing::info!(session_id, "transcript typing completed");
                    }
                }
                Ok(TypistEvent::Rejected { session_id, reason: TypistFailure::FocusChanged }) => {
                    if typing_session == Some(session_id) {
                        typing_session = None;
                        overlay.set_primary(OverlayPrimary::Hidden);
                        overlay.notify_warning("Typing stopped: active window changed", Duration::from_secs(5));
                        tray.set_status(if config.hotkey_enabled { TrayStatus::Ready } else { TrayStatus::Disabled });
                        tracing::warn!(session_id, "typing stopped because focus changed");
                    }
                }
                Ok(TypistEvent::Rejected { session_id, reason: TypistFailure::SendFailed(error) }) => {
                    if typing_session == Some(session_id) {
                        typing_session = None;
                        overlay.set_primary(OverlayPrimary::Hidden);
                        overlay.notify_error("Typing failed - see log", Duration::from_secs(7));
                        tray.set_status(TrayStatus::Error);
                        tracing::error!(session_id, %error, "typing stopped because text injection failed");
                    }
                }
                Err(_) => {}
            },
            recv(tray_rx) -> message => match message {
                Ok(TrayCommand::OpenSettings) => {
                    if let Err(error) = uvox::gui::show_settings_on_event_loop() {
                        overlay.notify_error("Settings window failed - see log", Duration::from_secs(7));
                ...
                Ok(TrayCommand::ToggleHotkey) => {
                    ...
                    tray.set_status(if config.hotkey_enabled { TrayStatus::Ready } else { TrayStatus::Disabled });
                    overlay.notify_info(
                        if config.hotkey_enabled { "Hotkey enabled" } else { "Hotkey disabled" },
                ...
                Ok(TrayCommand::ReloadConfig) => {
                    match AppConfig::load() {
                        Ok(new_config) => {
                            ...
                            tray.set_status(if config.hotkey_enabled { TrayStatus::Ready } else { TrayStatus::Disabled });
                            overlay.notify_info("Settings applied", Some(Duration::from_secs(1)));
                        ...
                        Err(error) => {
                            tray.set_status(TrayStatus::Error);
                            overlay.notify_error("Settings reload failed - see log", Duration::from_secs(7));
                ...
                Ok(TrayCommand::TestModel) => {
                    ...
                    overlay.notify_info("Testing speech model...", None);
                    ...
                    match uvox::models::smoke_test_model(&config.parakeet_runtime_dir_path(), &config.parakeet_model_path(), &audio) {
                        Ok(text) => {
                            tray.set_status(if config.hotkey_enabled { TrayStatus::Ready } else { TrayStatus::Disabled });
                            overlay.notify_info("Speech model test passed", Some(Duration::from_secs(2)));
                            tracing::info!(text, "tray model test passed");
                        }
                        Err(error) => {
                            tray.set_status(TrayStatus::Error);
                            overlay.notify_error("Speech model test failed - see log", Duration::from_secs(7));
            ...
            recv(control_rx) -> message => match message {
                Ok(ControlCommand::UnloadParakeetContext) => {
                    if active.is_none()
                        && typing_session.is_none()
                        && parakeet.as_ref().map_or(false, |engine| engine.is_context_loaded())
                    {
    ...
    if let Some(session_id) = typing_session {
        typist.cancel(session_id);
    }
    overlay.hide();
    tray.shutdown();
