...
The production implementation is in `src/capture/ipc_server.rs`, `src/bin/simple-stt-ctl.rs`, and `ahk/lib/IpcClient.ahk`. The AHK client launches one-shot helpers asynchronously and polls process completion with `SetTimer()`, so hotkeys and GUI callbacks never wait on a socket read.
